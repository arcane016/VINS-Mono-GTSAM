...
// 12345 #include "kimera-vio/utils/Timer.h"
...
namespace cpp_netvlad
{
  ...
  void NetVLAD::transform(const cv::Mat &img1, at::Tensor &rep)
  {
    // 12345 auto tic = VIO::utils::Timer::tic();
    TicToc t_transform;
    // t_transform.tic();
    ...
    // LOG(WARNING) << "NetVLAD img input" << img1.rows<<","<< img1.cols;
    cv::Mat img;
    cv::resize(img1, img, cv::Size(128 ,128), cv::INTER_LINEAR);
    at::Tensor tensor_img = torch::from_blob(img1.data, {img.channels(), img.rows, img.cols}, at::TensorOptions(at::kByte));
    ...
    tensor_img = (tensor_img / 255.f - at::tensor({0.485, 0.456, 0.406}).to(DTYPE).expand({1, 1, 1, 3}).permute({0, 3, 1, 2})) / at::tensor({0.229, 0.224, 0.225}).to(DTYPE).expand({1, 1, 1, 3}).permute({0, 3, 1, 2});
    inputs.push_back(tensor_img);

    // Run NetVLAD
    at::Tensor output = script_net_.forward(inputs).toTensor();
    // LOG(WARNING) << "NetVLAD o/p size " <<img.rows<<","<<img.cols;
    ...
    rep = output.squeeze();
    // 12345 auto duration = VIO::utils::Timer::toc(tic);
    // std::cout << "NetVLAD tf took " << t_transform.toc() << "ms\n";
    // LOG(WARNING) << "NetVLAD res size is " << rep.size(0) << ", type= "<<rep.dtype();
  }
                      ...
                      const int max_results, const int max_id) const
  {
    ...
    // 12345 auto duration = VIO::utils::Timer::toc(tic);
    // std::cout << "NetVLAD query took " << t_transform.toc() << "ms\n";
    // std::cout << "NetVLAD query scores: " << ((query_results.size() > 0) ? query_results[0].Score : -2.) << " " << ((query_results.size() > 1) ? query_results[1].Score : -2)<<"\n";
  }
